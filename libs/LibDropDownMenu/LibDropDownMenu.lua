...
Lib.UIDROPDOWNMENU_MINBUTTONS = 8;
Lib.UIDROPDOWNMENU_MAXBUTTONS = 8;
...
function Lib.UIDropDownMenu_CreateFrames(level, index)
	while ( level > Lib.UIDROPDOWNMENU_MAXLEVELS ) do
		...
		for i=Lib.UIDROPDOWNMENU_MINBUTTONS+1, Lib.UIDROPDOWNMENU_MAXBUTTONS do
...
function Lib.UIDropDownMenu_AddSeparator(info, level)
	info.text = nil;
	info.hasArrow = false;
	info.dist = 0;
	info.isTitle = true;
...
function Lib.UIDropDownMenu_AddButton(info, level)
	...
	if ( info.hasColorSwatch ) then
		_G["LibDropDownMenu_List"..level.."Button"..index.."ColorSwatch".."NormalTexture"]:SetVertexColor(info.r, info.g, info.b);
		button.r = info.r;
		button.g = info.g;
		button.b = info.b;
		colorSwatch:Show();
...
function Lib.UIDropDownMenu_Refresh(frame, useValue, dropdownLevel)
	...
	for i=1, Lib.UIDROPDOWNMENU_MAXBUTTONS do
		button = _G["LibDropDownMenu_List"..dropdownLevel.."Button"..i];
		checked = nil;
		...
		if not button.notCheckable and button:IsShown() then
			...
			checkImage = _G["LibDropDownMenu_List"..dropdownLevel.."Button"..i.."Check"];
			uncheckImage = _G["LibDropDownMenu_List"..dropdownLevel.."Button"..i.."UnCheck"];
			if ( checked ) then
		...
		if ( button:IsShown() ) then
			width = Lib.UIDropDownMenu_GetButtonWidth(button);
			if ( width > maxWidth ) then
				maxWidth = width;
			end
		end
		...
		if ( level == 1 ) then
			...
			if ( not anchorName ) then
				...
				if ( dropDownFrame.relativeTo ) then
					...
					relativeTo = Lib.UIDROPDOWNMENU_OPEN_MENU:GetName().."Left";
				end
...
function Lib.UIDropDownMenu_JustifyText(frame, justification)
